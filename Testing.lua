-- // Delta Mobil – Rivals: Panel + ESP (Renkli) + Aimbot (Tahminli) + Speed Slider
...
-- ==============================================
-- ESP (FindPartOnRayWithIgnoreList)
-- ==============================================
local ESPData = {}
...
local function createPanel()
    ...
    local function updateSlider(relX)
        ...
        local clamped = math.clamp(relX, 0, tw)
        local frac = clamped / tw
        cfg.speed_value = math.floor(minSpeed + (maxSpeed - minSpeed) * frac + 0.5)
        local realSpeed = cfg.speed_value * 2
        sliderLabel.Text = "Gerçek Hız: " .. realSpeed .. " (" .. cfg.speed_value .. ")"
        handle.Position = UDim2.new(0, clamped - 11, 0.5, -11)
        sliderFill.Size = UDim2.new(0, clamped, 1, 0)
